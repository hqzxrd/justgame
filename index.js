...
export class Vector2 {
    ...
    clamp(...args) {
        ...
        switch (args.length) {
            ...
            case 1:
                args[0] >= 0
                    ? this.set(Math.min(Math.max(this.x, 0), args[0]), Math.min(Math.max(this.y, 0), args[0]))
                    : this.set(Math.max(Math.min(this.x, 0), args[0]), Math.max(Math.min(this.y, 0), args[0]));
                break;
            case 3:
            case 2:
                args[1] >= args[0]
                    ? this.set(Math.min(Math.max(this.x, args[0]), args[1]), Math.min(Math.max(this.y, args[0]), args[1]))
                    : this.set(Math.min(Math.max(this.x, args[1]), args[0]), Math.min(Math.max(this.y, args[1]), args[0]));
                break;
            default:
                this.set(args[1] >= args[0]
                    ? Math.min(Math.max(this.x, args[0]), args[1])
                    : Math.min(Math.max(this.x, args[1]), args[0]), args[3] >= args[2]
    ...
    static fromAngle(angleOfRadians, length = 1) {
        return length <= 0
            ? new Vector2()
            : new Vector2(Math.cos(angleOfRadians) * length, Math.sin(angleOfRadians) * length);
    }
    /**
     * @returns {number} an unsigned angle (from 0 to PI radians) between 'vec1' and 'vec2'.
     */
    static angleBetween(vec1, vec2) {
        return Math.acos((vec1.x * vec2.x + vec1.y * vec2.y) / (Math.sqrt(Math.pow(vec1.x, 2)
